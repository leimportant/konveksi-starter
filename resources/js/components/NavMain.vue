...
const iconMap: Record<string, () => Promise<Component>> = {
    ...
    DollarSignIcon: () => import('lucide-vue-next').then((m) => m.DollarSignIcon),
    CreditCardIcon: () => import('lucide-vue-next').then((m) => m.CreditCardIcon),
    SoupIcon: () => import('lucide-vue-next').then((m) => m.SoupIcon),
...
<template>
    <SidebarGroup class="max-h-[calc(100vh-4rem)] overflow-y-auto px-3 py-2 text-gray-700 dark:text-gray-200">
                    ...
                    >
                        <div class="flex items-center space-x-3">
                            ...
                            <span class="flex items-center gap-1 text-gray-700 dark:text-gray-200">
                                {{ item.title }}
                                <span
                                    v-if="item.href === '/order-request' || item.href === '/customers'"
                        ...
                        <template v-for="child in item.children" :key="child.title">
                            <SidebarMenuItem class="relative">
                                ...
                                >
                                    ...
                                    <span
                                        v-if="child.href === '/order-request' || child.href === '/customers'"
                                        class="ml-1 rounded-full bg-red-500 px-2 py-0.5 text-xs font-bold text-white"
                                    >
