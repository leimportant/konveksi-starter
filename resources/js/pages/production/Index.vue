...
const clickCreate = () => {
  ...
  router.visit(
    `/production/${props.activity_role}/create?returnUrl=${encodeURIComponent(returnUrl)}`
  );
};
...
const clickEdit = (item: any) => {
    ...
    router.visit(`/production/${item.activity_role_id}/edit/${item.id}?returnUrl=${encodeURIComponent(returnUrl)}`);
};
            ...
            <div class="flex flex-col gap-3">
                <div class="flex min-w-0 flex-1 flex-col">
                    ...
                    <div class="flex gap-3">
                        <div class="flex min-w-0 flex-1 flex-col">
                            <label class="mb-1 text-xs font-medium text-gray-600">Start Date</label>
                            <input
                                type="date"
                                v-model="startDate"
                        ...
                        <div class="flex min-w-0 flex-1 flex-col">
                            <label class="mb-1 text-xs font-medium text-gray-600">End Date</label>
                            <input
                                type="date"
                                v-model="endDate"
                                class="w-full max-w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                ...
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex flex-1 items-center gap-2">
                        <Button
                            v-if="props.isCreate === 'Y'"
                            @click="clickCreate()"
                            class="flex h-10 flex-shrink-0 items-center gap-2 rounded-md bg-indigo-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500"
                        >
                            <Plus class="h-4 w-4" />
                            Tambah Data
                        </Button>
                        ...
                        <Input
                            v-model="searchQuery"
                            placeholder="Search..."
                            class="h-10 min-w-0 flex-1 rounded-md border border-gray-300 px-3 py-1 text-sm shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                            @keyup.enter="fetchData(1)"
                        />

                        <Button
                            @click="fetchData(1)"
                            class="h-10 flex-shrink-0 rounded-md bg-green-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-green-700 focus:ring-2 focus:ring-green-500"
                        >
                            Cari
                        </Button>
                    </div>
                                ...
                                >
                                    ...
                                    <td class="whitespace-nowrap px-3 py-2 align-top">
                                        ...
                                        >
                                            <!-- Loop over items -->
                                            <div
                                                v-for="(size, index) in item.items"
                                                :key="index"
                                            ...
                                            <div
                                                ...
                                                class="order-t mt-0.5 flex items-center justify-between rounded-md border-gray-300 bg-white px-1.5 py-0.5 pt-0.5 text-[11px] font-semibold text-gray-800 shadow-sm"
                                            >
                                                <span>Total</span>
                                    ...
                                    <td class="px-3 py-2 align-top">
                                        <div class="flex justify-end gap-1 sm:gap-2">
                                            <Button
                                                v-if="props.isCreate === 'Y' && isOwner == 'Y' && (item.status === 1 || item.status === 3)"
                                                variant="ghost"
                                                size="icon"
