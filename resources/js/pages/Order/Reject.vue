...
const fetchOrderDetails = async () => {
    if (props.order && props.order.id) {
        try {
            const response = await axios.post(`/api/orders/${props.order.id}/status/rejected`);
            order.value = response.data.order; // Assuming the API returns order data under 'order' key
            message.value = response.data.message || 'Order rejected successfully!';
        } catch (error) {
        ...
        message.value = 'No order ID provided for rejection.';
        order.value = null;
    }
};
