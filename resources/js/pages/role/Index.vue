...
const currentRole = ref<{ id: number; name: string } | null>(null);
...
const handleUpdate = async () => {
  if (!currentRole.value || !form.name) return toast.error("Name is required");

  try {
    await roleStore.updateRole(currentRole.value.id, form.name);
    ...
    const nameError = error?.response?.data?.errors?.name?.[0];
    toast.error(nameError || "Failed to update role");
  }
};
